...
3. You receive back a **URI** (the location of your content) and a **digestMultibase** (a multibase-encoded multihash fingerprint to verify it has not changed)

```mermaid
flowchart LR
    A[Your Data / File] --> B[Store]
    B --> C[URI + digestMultibase]
```
...
4. You receive back a **URI**, a **digestMultibase**, and a **decryption key**

```mermaid
flowchart LR
    A[Your Data / File] --> B[Encrypt]
    B --> C[Store]
    C --> D[URI + digestMultibase + Decryption Key]
```
